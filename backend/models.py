...
class User(db.Model):
    ...
    last_login_at = db.Column(db.DateTime)  # Legacy fallback support
    ...
    def check_password(self, password: str) -> bool:
        ...
        # 2. Try old Werkzeug hash
        try:
            if 'pbkdf2' in self.password_hash or 'scrypt' in self.password_hash:
                if check_password_hash(self.password_hash, password):
                    # 3. Success -> migrate to bcrypt hash and save automatically
    ...
    def to_dict(self):
        ...
        if email_val and email_val.endswith('@buysmart.placeholder'):
            email_val = None
        return {
            'id': self.id,
            'email': email_val,
...
class TokenBlocklist(db.Model):
    """Table for storing revoked/blacklisted JWTs"""
...
class AnalyticsCounter(db.Model):
    """Lightweight key-value table for global analytics counters (e.g. recommendations_served)"""
...
class UserPreference(db.Model):
    """User preference profile tracking affinities for personalization"""
    __tablename__ = 'user_preferences'
    
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), unique=True, nullable=False, index=True)
    preferred_categories = db.Column(db.JSON)  # Dict of {category: weight}
    preferred_brands = db.Column(db.JSON)      # Dict of {brand: weight}
    preferred_platforms = db.Column(db.JSON)   # Dict of {platform: weight}
    preferred_price_min = db.Column(db.Float, default=0.0)
    preferred_price_max = db.Column(db.Float, default=1000000.0)
...
class RecommendationFeedback(db.Model):
    """Tracks user feedback on recommendations (like, not_interested, save_for_later)"""
    __tablename__ = 'recommendation_feedback'
    
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    product_id = db.Column(db.Integer, db.ForeignKey('products.id', ondelete='CASCADE'), nullable=False, index=True)
    feedback_type = db.Column(db.String(50), nullable=False, index=True)  # 'like', 'not_interested', 'save_for_later'
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    user = db.relationship('User', backref=db.backref('recommendation_feedbacks', lazy=True))
    product = db.relationship('Product')

    __table_args__ = (
        db.UniqueConstraint('user_id', 'product_id', name='uq_user_product_feedback'),
    )
...
class AISearchEvent(db.Model):
    """Stores AI natural language search queries, rewrites, confidence, and feedback"""
    __tablename__ = 'ai_search_events'

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=True, index=True)
    query = db.Column(db.String(500), nullable=False)
    rewritten_query = db.Column(db.String(500), nullable=True)
    confidence_score = db.Column(db.Float, nullable=True)
    extracted_intent = db.Column(db.JSON)  # Dict containing category, brand, budget_min, budget_max, purpose, features, etc.
    search_explanation = db.Column(db.Text)
    results_count = db.Column(db.Integer, default=0)
    is_success = db.Column(db.Boolean, default=True, index=True)
    feedback = db.Column(db.String(30), nullable=True, index=True)  # 'helpful' or 'not_helpful'
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)

    user = db.relationship('User', backref=db.backref('ai_search_events', lazy=True))
    ...
    def to_dict(self):
        return {
            ...
            'extracted_intent': self.extracted_intent or {},
...
class AISearchCache(db.Model):
    """Caches AI intent extraction results for 24 hours to optimize token cost"""
    __tablename__ = 'ai_search_cache'

    query_hash = db.Column(db.String(64), primary_key=True)
    query = db.Column(db.String(500), nullable=False)
    intent_json = db.Column(db.JSON, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    expires_at = db.Column(db.DateTime, nullable=False, index=True)
